...
export class JourPage implements OnInit {
  ...
  date='';
  Kuhlmittel='';
  constructor(private emailComposer: EmailComposer, private route : Router, private file :File, private fileOpner: FileOpener, private pdfGenerator: PDFGenerator,
    private  alertCtrl : AlertController, private fileOpener:FileOpener, private plateform : Platform ) {
  ...
  pdfDownload() {
    ...
    const docDef = {
      ...
      content: [
        ...
        {text: 'Monatlische Wartung von Hauptmaschine 1 ',fontSize: 25,margin: [0, 45, 0, 0], bold: true, Style: 'subheader', alignment: 'center', },
        ...
        {text: 'Datum :',margin: [0, 45, 0, 0], fontSize:18},
                ...
                {text: 'Teppich :',margin: [0, 45, 0, 0], fontSize:18},
        {text: 'Gewechselt',margin: [150, -20, 0, 0], fontSize:16,bold:true},
       
{text: 'Schrauben am Rollenregal :',margin: [0, 45, 0, 0], fontSize:18},
        {text: 'Kontrolliert',margin: [250, -20, 0, 0], fontSize:16,bold:true},
  ...
  /*    this.pdfOjb= pdfMake.createPdf(docDef);
     ...
     /* let options = {
      ...
      this.pdfGenerator.fromData(pdfBlock.innerHTML, options).
        then(resolve => {
          console.log(resolve);
    ...
    const docDef = {
      ...
      content: [
        ...
        {text: 'Datum :',margin: [0, 45, 0, 0], fontSize:18},
                {text: this.Datum,margin: [150, -20, 0, 0], fontSize:16,bold:true},
                {text: 'Teppich :',margin: [0, 45, 0, 0], fontSize:18},
        {text: this.Teppich,margin: [150, -20, 0, 0], fontSize:16,bold:true},
       
{text: 'Schrauben am Rollenregal :',margin: [0, 45, 0, 0], fontSize:18},
        {text: this.Kuhlmittel,margin: [250, -20, 0, 0], fontSize:16,bold:true},
  ...
  /*    this.pdfOjb= pdfMake.createPdf(docDef);
      ...
      pdfMake.createPdf(docDef).getBlob(buffer => 
        ...
        this.file.resolveDirectoryUrl(this.file.externalCacheDirectory)
        .then(dirEntry => {
          this.file.getFile(dirEntry, Date.now().toLocaleString+'.pdf', { create: true })
            .then(fileEntry => {
              fileEntry.createWriter(writer => {
                writer.onwrite = () => {
                  this.fileOpener.open(fileEntry.toURL(), 'application/pdf')
                    ...
                    .then(res => {
                      let email = {
                        ...
                        subject: 'Tägliche Wartung',
                        body: 'Monatlische Wartung von Hauptmaschine 1',
                        isHtml: true
                      }
    ...
    /*  pdfMake.createPdf(docDef).getBuffer((buffer) => {
        ...
        let saveDir = this.file.externalCacheDirectory;
